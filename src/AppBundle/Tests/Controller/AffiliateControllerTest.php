...
class AffiliateControllerTest extends WebTestCase
{
    public function testAffiliateForm()
    {
        ...
        $this->assertEquals(
            'AppBundle\Controller\AffiliateController::newAction',
        ...
        $form = $crawler->selectButton('Submit')
                        ->form(
                            [
                                'affiliate[url]'           => 'http://www.example.com/',
                                'affiliate[email]'         => 'foo@example.com',
                                'affiliate[categories][1]' => $this->getProgrammingCategory()
                                                                   ->getId(),
                            ]
                        );
        ...
        $this->assertEquals(
            'AppBundle\Controller\AffiliateController::waitAction',
            $client->getRequest()->attributes->get('_controller')
        );
        $this->assertTrue(
        ...
        $this->assertTrue(
            $crawler->filter('#content h1')
                    ->text() == 'Your affiliate account has been created'
        );
        ...
        $crawler = $client->submit($form);
        $this->assertTrue(
            $crawler->filter('#affiliate_categories')
                    ->siblings()
                    ->first()
                    ->filter('.error_list')
                    ->count() == 1
